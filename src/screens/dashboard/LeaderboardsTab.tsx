...
function RankingMode({
  ...
  social,
  actions
}: Omit<LeaderboardsTabProps, "mode" | "socialInput">) {
  ...
  return (
    <DashboardCard>
      ...
      <View style={styles.segmented}>
        {leaderboardScopes.map((scopeOption) => (
          <Pressable
            ...
            accessibilityState={{ selected: scope === scopeOption.value }}
      ...
      <View style={styles.segmented}>
        {leaderboardWindows.map((windowOption) => (
          <Pressable
            ...
            accessibilityRole="button"
            accessibilityState={{ selected: window === windowOption.value }}
      ...
      <View style={styles.leaderboardBody}>
        {leaderboardLoading ? (
          <Text style={styles.smallCopy}>加载中…</Text>
                  ...
                  <View style={styles.rankActions}>
                    ...
                    {item.userId && item.userId !== leaderboard.currentUser?.userId ? (
                      <View style={styles.reactionRow}>
                        <Pressable
                          accessibilityRole="button"
                          accessibilityLabel={`给 ${item.displayName} 递纸巾`}
                        ...
                        <Pressable
                          accessibilityRole="button"
                          accessibilityLabel={`给 ${item.displayName} 点赞`}
...
function SocialMode({
  ...
  socialInput,
  actions
}: Omit<LeaderboardsTabProps, "mode" | "leaderboard" | "leaderboardLoading" | "window" | "scope">) {
  return (
    <DashboardCard>
...
export function LeaderboardsTab(props: LeaderboardsTabProps) {
  const target = props.mode === "social" ? "social" : "ranking";
  return (
    <View onLayout={(event) => props.onLandingLayout(target, event.nativeEvent.layout.y)}>
      {props.mode === "social" ? <SocialMode {...props} /> : <RankingMode {...props} />}
    </View>
  );
}
